...
class UserLevelController:
    ...
    def set_chapter_completed(user_id):
        try:
            ...
            chapter_exists = user_level_service._is_chapter_exists(user_id, chapter_sequence)
            if chapter_exists:
                chapter_progress = user_level["chapter_progress"][chapter_sequence]
                # 檢查章節是否已解鎖
                if chapter_progress.get("completed", False):
                    return {
                        "message": "該章節已設置完成"
                    }, 400
                highest_level = int(user_level['highest_level'])
                if (highest_level / 5 == chapter_sequence and highest_level % 5 == 0):
                    result = user_level_service.set_chapter_completed(user_id, chapter_sequence)
